...
class PIInitiatorDriver:
    ...
    def begin(self):
        yield self.pi.ale_l.i.eq(1)
        yield self.pi.ale_h.i.eq(0)
        yield Delay(1e-6)

    def read_burst_slow(self, start_address, word_count):
        yield self.pi.ale_l.i.eq(0)
        yield Delay(56e-9)
        yield self.pi.ad.i.eq((start_address >> 16) & 0xFFFF)
        yield Delay(56e-9)
        yield self.pi.ale_h.i.eq(1)
        yield Delay(56e-9)
        ...
        for i in range(word_count):
            yield self.pi.read.i.eq(1)
            yield Delay(304e-9)

            word = yield self.pi.ad.o
            yield self.pi.read.i.eq(0)
            yield Delay(64e-9)
